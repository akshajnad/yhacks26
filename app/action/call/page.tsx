...
export default function CallPage() {
  ...
  const [briefWithOverrides, setBriefWithOverrides] =
    useState<OutreachBrief | null>(null);

  const [callBrief, setCallBrief] = useState<CallBriefResponse | null>(null);
  const [elevenLabsPayload, setElevenLabsPayload] =
    useState<ElevenLabsCallPayload | null>(null);

  const [callLoading, setCallLoading] = useState(false);
  const [callError, setCallError] = useState<string | null>(null);
  const [callSuccess, setCallSuccess] = useState<{ toNumber: string } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const target =
      briefWithOverrides.contactTargets.find(
        (t) => t.audience === "provider",
  ...
  const target =
    briefWithOverrides?.contactTargets.find((t) => t.audience === "provider") ??
    briefWithOverrides?.contactTargets[0];
    ...
    return (
      <section className="mx-auto w-full max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">
          No analysis found for this tab. Open this page from the analysis
          result "Call Providers" action.
        </div>
      </section>
    );
  }
        ...
        <header className="flex justify-between items-start">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">
              Review the auto-generated call brief below and place a secure,
              automated call using ElevenLabs.
            </p>
          </div>
          {elevenLabsPayload && (
            <Button
              ...
              disabled={
                callLoading || !hasPhone || elevenLabsPayload.mode === "preview"
              }
            ...
            {elevenLabsPayload?.mode === "preview" && (
              <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3">
                <p className="text-sm text-amber-800">
                  ...
                  Set{" "}
                  <code className="rounded bg-amber-100 px-1.5 font-mono text-amber-900">
                    ELEVENLABS_API_KEY
                  </code>
                  ,{" "}
                  <code className="rounded bg-amber-100 px-1.5 font-mono text-amber-900">
                  ...
                  to enable live automated calls.
                </p>
              </div>
            )}
            {/* Call Notifications */}
            {callError && (
              <div className="flex items-start gap-2 rounded-md border border-red-200 bg-red-50 px-4 py-3">
                <p className="flex-1 text-sm text-red-700 font-medium">
                  Failed to place call: {callError}
                </p>
              </div>
            )}
            {callSuccess && (
              <div className="space-y-3">
                <div className="flex items-center gap-2 rounded-md border border-green-300 bg-green-100 px-4 py-3 shadow-sm">
                  ...
                  <p className="text-sm font-medium text-green-800">
                    Call successfully initiated to {callSuccess.toNumber}. The
                    ElevenLabs agent is calling them now.
                  </p>
                </div>
            ...
            {target && elevenLabsPayload?.recipient && (
              <div className="rounded-lg border border-[var(--border)] bg-white p-5">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  {!hasPhone && (
                    <Badge variant="destructive">No Phone Extracted</Badge>
                  )}
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-lg border border-[var(--border)] bg-slate-50 p-5">
                  ...
                  <textarea
                    ...
                    value={
                      elevenLabsPayload?.conversation?.firstMessage ||
                      callBrief.openingScript ||
                      ""
                    }
                    onChange={(e) => {
                      if (elevenLabsPayload) {
                        setElevenLabsPayload({
                          ...elevenLabsPayload,
                          conversation: {
                            ...elevenLabsPayload.conversation,
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <ScriptSection
                    title="Must Say Goals"
                    items={callBrief.mustSay}
                    color="blue"
                  />
                  {callBrief.legalPoints.length > 0 && (
                    <ScriptSection
                      title="Legal Points to Bring Up"
                      items={callBrief.legalPoints}
                      color="purple"
                    />
                  )}
                  <ScriptSection
                    title="Escalation Path if Unresolved"
                    items={callBrief.escalationPath}
                    color="amber"
                  />
                  {callBrief.prohibitedClaims.length > 0 && (
                    <ScriptSection
                      title="Do NOT Say (Risk)"
                      items={callBrief.prohibitedClaims}
                      color="red"
                    />
                  )}
                </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 text-sm">
                    <div>
                      ...
                      <p className="font-mono font-medium text-slate-900">
                        {callBrief.referenceNumbers.memberID || "—"}
                      </p>
                    </div>
                    <div>
                      <span className="text-slate-500 text-xs">Claim #</span>
                      <p className="font-mono font-medium text-slate-900">
                        {callBrief.referenceNumbers.claimNumber || "—"}
                      </p>
                    </div>
                    <div>
                      <span className="text-slate-500 text-xs">Account #</span>
                      <p className="font-mono font-medium text-slate-900">
                        {callBrief.referenceNumbers.accountNumber || "—"}
                      </p>
                    </div>
                  </div>
                </div>
