      ...
      <div className="space-y-6 rounded-xl border border-[var(--border)] bg-white p-6">
        <header>
          ...
          <p className="mt-2 text-sm text-slate-600">
            Generate, edit, and send a dispute email using your authorized Gmail
            account.
          </p>
          {summary ? (
        ...
        <div className="rounded-lg border border-[var(--border)] bg-slate-50 p-4 text-sm">
          ...
          <p className="mt-1 text-slate-600">
            {gmailConnectedEmail
              ? `Signed in as ${gmailConnectedEmail}`
              : "No Google account connected."}
          </p>
          {gmailConnectedEmail && !isExpectedSender ? (
            <p className="mt-2 text-red-700">
              This feature only sends when the connected account is
              akshajnadn@gmail.com.
            </p>
          ) : null}
        ...
        {status === "loading" ? (
          <p className="text-sm text-slate-600">
            Generating draft from analysis…
          </p>
        ) : null}
        ...
        {sendState ? (
          <div className="space-y-3">
            <p className="text-sm font-medium text-slate-800 border-l-4 border-slate-900 pl-3 py-1 bg-slate-50">
              {sendState}
            </p>
