...
export function AnalysisResultDisplay({ result }: AnalysisResultProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[var(--border)] bg-slate-50 px-4 py-3 shadow-sm">
        <div>
      ...
      <div className="space-y-4">
        <SectionWrapper delay={0}>
          <CollapsibleCard title="Extracted Fields" icon={<DocumentIcon />}>
        ...
        <SectionWrapper delay={2}>
          <CollapsibleCard title="Relevant Legal Protections" icon={<ScaleIcon />} defaultOpen={(result.laws ?? []).length > 0}>
            {(result.laws ?? []).length > 0 ? (
              ...
              <p className="text-sm text-slate-600">No specific legal protections were detected for this case.</p>
            )}
          </CollapsibleCard>
        </SectionWrapper>
          ...
          <CollapsibleCard title="Take Action" icon={<CheckIcon />} className="relative">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <GlowCard className="p-4" customSize>
                ...
                <Button
                  asChild
                  className="mt-3 w-full gap-2 bg-blue-600 text-white transition-transform duration-300 hover:scale-[1.02] hover:bg-blue-700"
      ...
      <details className="rounded-xl border border-[var(--border)] bg-white shadow-sm">
        <summary className="cursor-pointer list-none px-4 py-3 text-sm font-semibold text-slate-900">Additional case details</summary>
      ...
      {actionTaken && (
        <div className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-4 shadow-sm animate-fade-up">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-relaxed text-blue-700">
              Outreach initiated. Our agent has successfully scheduled an automated follow-up in <span className="font-bold underline">7 days</span>.
            </p>
          </div>
        </div>
      )}
...
function SectionWrapper({ delay, children }: { delay: number; children: ReactNode }) {
  return (
    <div className="animate-fade-up" style={{ animationDelay: `${delay * 90}ms` }}>
      {children}
    </div>
  )
}

function SummaryCard({ title, value, subtext, emphasis = false }: { title: string; value: string; subtext: string; emphasis?: boolean }) {
  return (
    <div className="rounded-xl border border-[var(--border)] bg-slate-50 p-4 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md">
...
function Field({ label, value, isHighlight = false }: { label: string; value: string | null; isHighlight?: boolean }) {
  return (
    <div>
...
function IssueCard({ issue, potentialSavings }: { issue: { type: IssueType; severity: IssueSeverity; description: string }; potentialSavings: number | null }) {
  const isError = issue.severity === "error"
  const meta = ISSUE_REASONING[issue.type]

  return (
    <div className={`rounded-lg border bg-white p-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md ${isError ? "border-red-200" : "border-amber-200"}`}>
