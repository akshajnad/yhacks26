    ...
    <header className="sticky top-0 z-50 border-b border-[var(--border)] bg-white/95">
      <div className="mx-auto flex h-14 w-full max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-2 text-base font-semibold tracking-tight text-slate-900">
          <span>Redline</span>
          <span className="hidden text-xs font-medium text-slate-500 sm:inline">Medical Billing Audit</span>
        </Link>
          ...
          {!isLoading && user ? (
            <>
              ...
              <div className="hidden items-center gap-2 rounded-md border border-[var(--border)] bg-slate-50 px-2.5 py-1 sm:flex">
                ...
                <span className="max-w-36 truncate text-xs font-medium text-slate-700" title={displayName}>
                  {displayName}
                </span>
              </div>
              <button
